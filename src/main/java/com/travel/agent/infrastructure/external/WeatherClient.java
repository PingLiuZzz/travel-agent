 ...
 *
 * <p>查询流程：城市名 → 经纬度(geocoding) → 实时天气(forecast)。
 * weather_code 采用完整 WMO 标准代码表映射中文描述，并按天气类型 + 温度给出个性化出行建议。
 * JSON 用 Map 解析（一次性结构，不为它定义 DTO，符合 KISS）。
 */
@Component
public class WeatherClient {
    ...
    private double[] geocode(String city) {
        Map<String, Object> resp = restClient.get()
                .uri(uri -> uri.scheme("https").host("geocoding-api.open-meteo.com")
                        .path("/v1/search")
    ...
    private WeatherInfo fetchForecast(String city, double latitude, double longitude) {
        Map<String, Object> resp = restClient.get()
                .uri(uri -> uri.scheme("https").host("api.open-meteo.com")
                        .path("/v1/forecast")
    ...
    /**
     * WMO 天气代码 → 中文描述（完整标准代码表）。
     * 参考Open-Meteo 文档的 WMO Weather interpretation codes (WW)。
     */
